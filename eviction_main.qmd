...
# Save figures.
opts_chunk$set(fig.path = "final_figures/")
```
...
Usually, delinquent properties are not considered delinquent if they have overdue payments, it is only for properties where penalties are assessed, and because it is just a marginal 8 properties that were overdue rather than delinquent they were removed.
...
The dispersion ratio exceeds 1, indicating overdispersion that violates the Poisson assumption of equal mean and variance. This justifies the use of Negative Binomial regression over Poisson regression. There are also 37% zero filings, so perhaps in the future a Zero Inflated Negative Binomial regression could potentially perform better even though Negative Binomial itself can handle zeros.
...
Weekly distribution is severely zero-inflated at 70.1%, so it would not be a methodologically sound choice to use Negative Binomial with the current toolkit. While it would be more useful and actionable to provide more granular weekly predictions, this is more doable with the Zero-Inflated Negative Binomial regression in a future study.
...
Another indication that the quantiles are very inflated at 0 as median is 3, then 5 for 75th percentile, a jump to 10 at 95th percentile, another jump to 16 at the 99th percentile, and a whopping 694 jump to the max.
...
Temporal lags capture the momentum or inertia in eviction filing patterns. A tract with high filings last month is likely to have elevated filings this month.
...
```
First month has no spatial lag because of no prior month data!
...
Worst MAE degradation across models is 19.44%, so good generalization. Looks like the test MAE for the best model, which is the ACCS one, is off by 1.77 filings and accounts for 57% of variation. This is a good start and quite good considering we are essentially measuring a response variable that is very tied to human behavior.
...
Model does not perform as well for tracts with higher Black populations when it comes to predicting eviction filings as opposed to other tracts, with white tracts having the least MAE. Future modifications need to be made to account for Black communities who account for the majority of Philadelphia's population.
