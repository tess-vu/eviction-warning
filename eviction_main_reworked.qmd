...
```{r model-2}
model_nb_2 <- glmmTMB(filings_count_capped ~
...
```{r model-3}
model_nb_3 <- glmmTMB(filings_count_capped ~
                       # 1st model.
                       filings_ma3 + moratorium_active + factor(month_num) + is_extreme_spike +
                       # 2nd model.
                       spatial_lag_filings + delinquent_prop_count +
...
```{r model-4}
model_nb_4 <- glmmTMB(filings_count_capped ~
                       # 1st model.
                       filings_ma3 + moratorium_active + factor(month_num) + is_extreme_spike +
                       # 2nd model.
                       spatial_lag_filings + delinquent_prop_count +
...
get_theta <- function(model) {
  suppressWarnings(as.numeric(sigma(model)))
}
