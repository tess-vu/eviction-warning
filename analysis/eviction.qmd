...
# Write model_predictions.csv
city_median_delinquent <- median(df_tax$delinquent_prop_count, na.rm = TRUE)

model_predictions <- df_test_risk %>%
  mutate(
    action_tier = case_when(
      predicted >= 12 ~ "Deploy canvassers",
      predicted >= 8  ~ "Direct mail",
      predicted >= 5  ~ "Legal aid pop-up",
      TRUE            ~ "Monitor"
    ),
    spatial_lag_tier = case_when(
      is.na(spatial_lag_filings) ~ "N/A",
      spatial_lag_filings > 5   ~ "High",
      spatial_lag_filings >= 2  ~ "Moderate",
      TRUE                      ~ "Low"
    ),
    tax_stress = if_else(
      !is.na(delinquent_prop_count) & delinquent_prop_count > city_median_delinquent,
      "Yes",
      "No"
    ),
    bias = predicted - filings_count
  ) %>%
  select(
    GEOID,
    date,
...
required_cols <- c(
  "GEOID", "date", "racial_majority",
  "filings_count", "predicted", "neighborhood"
)

missing_cols <- setdiff(required_cols, names(model_predictions))

if (length(missing_cols) > 0) {
  stop(sprintf(
    "export-predictions: missing required columns: %s",
    paste(missing_cols, collapse = ", ")
  ))
}

output_dir <- "../outputs"
output_path <- file.path(output_dir, "model_predictions.csv")

if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}

write_csv(model_predictions, output_path)

cat(sprintf(
  "export-predictions: wrote %d rows × %d cols to %s\n",
  nrow(model_predictions),
  ncol(model_predictions),
  output_path
))
cat(sprintf(
  "  Date Range: %s → %s\n",
  format(min(model_predictions$date), "%Y-%m-%d"),
  format(max(model_predictions$date), "%Y-%m-%d")
))
cat(sprintf(
  "  Racial Groups: %s\n",
  paste(sort(unique(model_predictions$racial_majority)), collapse = ", ")
))
cat(sprintf(
  "  Critical-risk tracts (predicted >= 12): %d\n",
  sum(model_predictions$predicted >= 12, na.rm = TRUE)
))
```
