...
# Data file paths
path_bus_ridership <- "../data/septa/Bus_Ridership_by_Census_Tract.csv"
path_pa_tracts <- "../data/pa_tracts/pa_tracts.shp"
path_transit_routes <- "../data/septa/Transit_Routes_Spring_2025/Transit_Routes.shp"
path_stop_summary <- "../data/septa/Fall_2023_Stop_Summary_Bus.csv"
path_route_ridership <- "../data/septa/Average_Daily_Ridership_By_Route.csv"
path_mode_ridership <- "../data/septa/Average_Daily_Ridership_By_Mode.csv"
path_rr_stations <- "../data/septa/Regional_Rail_Station_Summary.csv"
```

```{r data-load-dim}
df_raw <- read_csv(path_bus_ridership, show_col_types = FALSE)
...
df_clean <- df_raw %>%
  ...
  mutate(
    Is_Weekday = if_else(Day_of_Week == "Weekday", 1L, 0L),
    Day_Type = factor(Day_of_Week, levels = c("Weekday", "Weekend", "Sunday"))
  ) %>%
  # Pandemic period indicator for structural break analysis.
  mutate(
    Period = case_when(
...
# Zero-ridership tracts may indicate service gaps or data collection issues.
...
# Pivot On_/Off_ into long format for faceted comparison.
df_long <- df_clean %>%
...
# Duplicate with log scale for second facet column.
...
yoy_growth <- annual_trend %>%
  ...
  mutate(
    ...
    Growth_Type = if_else(YoY_Change >= 0, "Growth", "Decline")
  ) %>%
...
# Minimum 100 boardings in 2019 to avoid ratio distortion from low-volume tracts.
...
# First year for each tract will have NA lag — expected.
...
# Higher CV = less predictable ridership; useful for identifying volatile tracts.
...
zero_patterns <- df_clean %>%
  mutate(Is_Zero = if_else(On_ == 0 & Off_ == 0, 1L, 0L)) %>%
...
service_coverage <- df_clean %>%
  mutate(Has_Service = if_else(On_ > 0 | Off_ > 0, 1L, 0L)) %>%
  group_by(Census_Tract_ID, County_Name) %>%
  summarize(
...
# Is_Weekday excluded — constant after weekday filter, gives SD warning.
cor_data <- df_clean %>%
  filter(Day_of_Week == "Weekday") %>%
...
```{r train-test-split}
# Temporal split avoids data leakage from random sampling.
...
cat(sprintf("\nOverlapping Observations: %d (should be 0)\n", length(overlap)))
```
...
summary_table %>%
  ...
  mutate(Value = if_else(
    Statistic %in% c("Mean_On", "Median_On", "SD_On", "Min_On", "Max_On"),
    round(Value, 1),
    round(Value, 0)
  )) %>%
  kable() %>%
...
tract_geo <- st_read(path_pa_tracts)

# FIPS codes for SEPTA five-county service area.
...
routes <- st_read(path_transit_routes)
```
...
spatial_plot_log_on <- ggplot(tract_map_data_with_ridership) +
  ...
  theme_void() +
  theme(
    plot.background = element_rect(fill = map_bg, color = map_bg),
    plot.title = element_text(face = "bold", size = 14)
  )
...
```{r load-stop-data}
df_stops <- read_csv(path_stop_summary, show_col_types = FALSE)
...
df_stops_clean <- df_stops %>%
  mutate(
    ...
    Route_Numeric = as.numeric(if_else(grepl("^[0-9]+$", Route), Route, NA_character_))
  ) %>%
...
median_weekday_on <- median(df_stops_clean$Weekday_On[df_stops_clean$Weekday_On > 0])

stop_dist_plot <- df_stops_clean %>%
  filter(Weekday_On > 0) %>%
  ggplot(aes(x = log10(Weekday_On + 1))) +
  geom_histogram(bins = 50, fill = "#3498DB", color = "white", alpha = 0.8) +
  geom_vline(xintercept = log10(median_weekday_on + 1),
             linetype = "dashed", color = "#E74C3C", linewidth = 1) +
  annotate("text",
           x = log10(median_weekday_on + 1),
           y = Inf,
           label = sprintf("Median = %.0f", median_weekday_on),
...
# Bounding box from stop coordinates — reused by later spatial plots.
...
```{r load-route-data}
df_route <- read_csv(path_route_ridership, show_col_types = FALSE)
...
```{r load-modal-data}
df_mode <- read_csv(path_mode_ridership, show_col_types = FALSE)
...
df_rr <- read_csv(path_rr_stations, show_col_types = FALSE)
...
# Commuter score: ratio of weekday-weekend imbalance, higher = more commuter-oriented.
